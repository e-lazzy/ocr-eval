...
void saveToCSV(const std::string& filename, int* stats, int total, std::vector<std::vector<SymbolChoice>>& symbolChoices) {
    ...
    outputFile << "OCR_DIRECT,OCR_FIRST,OCR_INLIST,OCR_OUTLIST,OCR_NAN" <<std::endl;
    

    // Écrire les données du premier tableau
    for (int i = 0; i < 5; ++i) {
        outputFile << stats[i] << "/" << total << ",";
        if (i<4){
          outputFile << "";
        }else{
          outputFile << std::endl;
    ...
    outputFile << "ImageNumber,Option,Probability" <<std::endl;
    //donnees deuxieme tableau
    
    int ImageNumber=0;
    bool firstOptionWritten=false;
    
    for (size_t i = 0; i < symbolChoices.size(); ++i) {
        for (size_t j = 0; j < symbolChoices[i].size(); ++j) {
          if (!firstOptionWritten){
            outputFile << ImageNumber <<","; // Image number
            ...
            outputFile <<symbolChoices[i][j].symbol << ",";
            outputFile << symbolChoices[i][j].probability << std::endl;
            if(!firstOptionWritten){
              firstOptionWritten=true;
           
          }
          
    }
    ImageNumber++;
    firstOptionWritten=false;
    }

    outputFile.close();
}

int getTotal(const int* stats, int size) {
    int total = 0;
  ...
  const int expected, const tesseract::PageSegMode psm,std::vector<std::vector<SymbolChoice>>& allSymbolChoices)
{
  ...
  if (res_it != 0) {
    ...
    do {
        ...
        for (auto timestep : *choiceMap) {
          ...
          if (timestep.size() > 0) {
            ...
            for (auto & j : timestep) {
              conf = 100.0 * j.second;
              word =  j.first;
              symbolChoices.push_back({word,conf});
      ...
      allSymbolChoices.push_back(symbolChoices);
      ImageNumber++;
